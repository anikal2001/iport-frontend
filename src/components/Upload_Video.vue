  ...
  <v-layout align-center justify-space-around row fill-height>
    <v-hover>
      ...
      >
        <form enctype="application/x-www-form-urlencoded" novalidate v-if="isInitial || isSaving">
          <div class="uploadbutton">
            ...
            <input
              ...
              :disabled="isSaving"
              multiple
              @change="NameOfFiles = $event.target.name; ListofFiles = $event.target.files;"
            />
            ...
            <v-card-title primary-title>
              <div>
                <span class="headline" style="padding:20px">
                  ...
                  <p v-if="isSaving">Uploading {{ ListofFiles.Length }} files...</p>
                </span>
              </div>
            </v-card-title>
          </div>
        ...
        <v-btn
          v-if="isInitial && ListofFiles.length>0"
          large
          @click="filesChange(NameOfFiles, ListofFiles)"
        >upload</v-btn>
    ...
    >
      ...
      <v-list>
        <v-list-tile v-for="ListofFile in ListofFiles" v-bind:key="ListofFile.name">
